...
def extract_text_from_book_using_textract(book_path):
    text: str = textract.process(
        book_path, encoding="utf-8"
    ).decode()  # decode using textract
...
def extract_main_content(text, start_marker="PROLOGUE", end_marker="ABOUT THE AUTHOR"):
    ...
    try:
        ...
        if start_index is None or end_index is None:
            raise ValueError(
                "Could not find valid marker positions with substantial content between them"
            )
...
def save_book(edited_text, book_title):
    with open(
        f"{TEMP_DIR}/{book_title}/converted_book.txt", "w", encoding="utf-8"
    ) as fout:
...
def main(book_title: str):
    ...
    if len(sys.argv) > 1:
        ...
        input_path = input(
            "\n📖 Enter the **path to the book file** (Press Enter to use default): "
        ).strip()
        if input_path:
    ...
    if text_decoding_option == "calibre":
        ...
        if is_calibre_installed:
            ...
            print(
                "⚠️ Calibre is not installed. Please install it first and make sure **calibre** and **ebook-convert** commands are available in your PATH."
            )
            return
    else:
    ...
    print(
        "✍️ Normalizing the text by replacing curly quotes and apostrophes with standard ASCII equivalents...\n"
    )
    ...
    text = (
        ...
        .replace("\u2019", "'")
        .replace("\u2018", "'")
    )  # Normalize text by replacing curly quotes and apostrophes with standard ASCII equivalents
    ...
    if have_to_extract_main_content == "yes":
        start_marker = input(
            "🔹 Enter the **start marker** for the main content (case-sensitive): Default is **PROLOGUE** :"
        ).strip()
        if not start_marker:
            start_marker = "PROLOGUE"
        end_marker = input(
            "🔹 Enter the **end marker** for the main content (case-sensitive): Default is **ABOUT THE AUTHOR** :"
        ).strip()
        if not end_marker:
            end_marker = "ABOUT THE AUTHOR"
        text = extract_main_content(
            text, start_marker=start_marker, end_marker=end_marker
        )
    ...
    with open(
        ...
        print(
            "🔍 Please manually review the converted book and remove any unnecessary content.\n"
        )
        ...
        print(
            "    (This script will identify characters and assign gender & age scores.)\n"
        )
